...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .faq-item{border:1px solid var(--panel-border); border-radius:var(--radius-md); background:var(--panel-2); overflow:hidden; transition:border-color .3s ease;}
  .faq-item.open{border-color:rgba(201,162,109,.4);}
  .faq-item .faq-q{
  ...
  .faq-item .faq-q .plus::before, .faq-item .faq-q .plus::after{
    content:""; position:absolute; background:var(--blue-light);
  }
  .faq-item .faq-q .plus::before{width:9px; height:1.4px;}
  .faq-item .faq-q .plus::after{width:1.4px; height:9px; transition:transform .25s ease;}
  .faq-item.open .faq-q .plus::after{transform:rotate(90deg); opacity:0;}
  .faq-item.open .faq-q .plus{border-color:var(--gold); background:rgba(201,162,109,.12);}
  .faq-a-wrap{max-height:0; overflow:hidden; transition:max-height .45s cubic-bezier(.4,0,.2,1);}
  .faq-item .faq-a{padding:0 24px 22px; font-size:14px; color:var(--muted); max-width:640px; min-height:1em;}
  .faq-item .faq-a.typing::after{
    content:"▍"; color:var(--gold-light); margin-left:2px; animation:faqCursor .8s steps(1) infinite;
  }
  @keyframes faqCursor{ 50%{ opacity:0; } }

  /* ============ INTERAÇÕES GERAIS ============ */
  .scroll-progress{position:fixed; top:0; left:0; width:100%; height:3px; z-index:150; background:transparent;}
  .scroll-progress-bar{height:100%; width:0%; background:linear-gradient(90deg, var(--gold), var(--blue-light)); box-shadow:0 0 10px rgba(122,162,255,.5);}

  .adv-card, .model-card, .format-card, .testi-card, .step{ position:relative; }
  .adv-card::before, .model-card::before, .format-card::before, .testi-card::before, .step::before{
    content:""; position:absolute; inset:0; border-radius:inherit; pointer-events:none;
    background:radial-gradient(260px circle at var(--mx,50%) var(--my,50%), rgba(201,162,109,.16), transparent 70%);
    opacity:0; transition:opacity .35s ease; z-index:0;
  }
  .adv-card:hover::before, .model-card:hover::before, .format-card:hover::before, .testi-card:hover::before, .step:hover::before{ opacity:1; }

  .btn{position:relative; overflow:hidden;}
  .ripple{
    position:absolute; border-radius:50%; background:rgba(255,255,255,.5);
    transform:scale(0); pointer-events:none; animation:rippleAnim .6s ease-out forwards;
  }
  @keyframes rippleAnim{ to{ transform:scale(2.6); opacity:0; } }

  .invite-mock{ transition:transform .12s ease-out; will-change:transform; }

  @media (prefers-reduced-motion: reduce){
    .btn, .invite-mock{ transition:none; }
  }
      ...
      <div class="faq-list">
        <div class="faq-item reveal">
          <button class="faq-q" aria-expanded="false">
            <span>Quanto tempo leva para ficar pronto?</span><span class="plus"></span>
          </button>
          <div class="faq-a-wrap">
            <p class="faq-a" data-full="Em média de 3 a 7 dias úteis após o briefing, dependendo da complexidade do projeto e do plano escolhido."></p>
          </div>
        </div>
        <div class="faq-item reveal">
          <button class="faq-q" aria-expanded="false">
            <span>Preciso entender de tecnologia para usar?</span><span class="plus"></span>
          </button>
          <div class="faq-a-wrap">
            <p class="faq-a" data-full="Não. Você só participa do briefing e recebe o link pronto — nós cuidamos de toda a parte técnica."></p>
          </div>
        </div>
        <div class="faq-item reveal">
          <button class="faq-q" aria-expanded="false">
            <span>Funciona direto pelo WhatsApp?</span><span class="plus"></span>
          </button>
          <div class="faq-a-wrap">
            <p class="faq-a" data-full="Sim. O convite pode ser enviado por link no WhatsApp, redes sociais ou e-mail, sem limite de convidados."></p>
          </div>
        </div>
        <div class="faq-item reveal">
          <button class="faq-q" aria-expanded="false">
            <span>Posso editar depois que o convite estiver pronto?</span><span class="plus"></span>
          </button>
          <div class="faq-a-wrap">
            <p class="faq-a" data-full="Pequenos ajustes de texto e data podem ser feitos após a entrega. Alterações maiores de design são orçadas à parte."></p>
          </div>
        </div>
        <div class="faq-item reveal">
          <button class="faq-q" aria-expanded="false">
            <span>Quais as formas de pagamento?</span><span class="plus"></span>
          </button>
          <div class="faq-a-wrap">
            <p class="faq-a" data-full="Combinamos a forma de pagamento diretamente no orçamento, de acordo com o plano e o prazo do seu evento."></p>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  ['openOrcamentoNav','openOrcamentoHero','openOrcamentoCta','openOrcamentoFooter'].forEach(id=>{
  ...
  /* ---------- SPOTLIGHT NOS CARDS ---------- */
  document.querySelectorAll('.adv-card, .model-card, .format-card, .testi-card, .step').forEach(card => {
  ...
  /* ---------- TILT NO CARD DO CONVITE (HERO) ---------- */
  const heroMock = document.querySelector('.invite-mock');
  if(heroMock && !reduceMotion){
    heroMock.addEventListener('mousemove', (e) => {
      const rect = heroMock.getBoundingClientRect();
      const px = (e.clientX - rect.left) / rect.width - 0.5;
      const py = (e.clientY - rect.top) / rect.height - 0.5;
      heroMock.style.transform = `perspective(900px) rotateY(${px * 9}deg) rotateX(${-py * 9}deg) translateY(-2px)`;
    });
    heroMock.addEventListener('mouseleave', () => { heroMock.style.transform = ''; });
  }

  /* ---------- FAQ COM EFEITO DE DIGITAÇÃO AO VIVO ---------- */
  document.querySelectorAll('.faq-item').forEach(item => {
    const btn = item.querySelector('.faq-q');
    const wrap = item.querySelector('.faq-a-wrap');
    const para = item.querySelector('.faq-a');
    ...
    function typeAnswer(){
      ...
      typeTimer = setInterval(() => {
        i++;
        para.textContent = fullText.slice(0, i);
    ...
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.faq-item.open').forEach(other => {
        if(other !== item){
          ...
          other.querySelector('.faq-a-wrap').style.maxHeight = '0px';
          clearInterval(typeTimer);
        }
      });
